...
class WorkOnSpectrum:
    def __init__(self, filelist, figure, ax, dustmap, settings, resid_corr, classifier, x):
        ...
        self.predict_from_mlp = theano.function(
            inputs=[self.index],
            outputs=[self.classifier.get_hidden_values(self.x),
                     self.classifier.get_reconstructed_input(self.classifier.get_hidden_values(self.x))
            ],
            givens={
    ...
    def work_on_spectrum(self, filename):
        ...
        flux_prediction, reconstruction = self.predict_from_mlp(predict_index)
        ...
        self.axarr.plot(self.wave_predict[predict_index], reconstruction, 'k-')
...
def main(args):
    ...
    # construct the SdA class
    if '--SdA' in args:
        # construct SdA as an MLP
        classifier = MLP(
            rng=rng,
            input=x,
            layer_params=layer_params,
