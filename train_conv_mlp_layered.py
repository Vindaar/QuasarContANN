#!/usr/bin/env python
"""
This script trains a convolutional neural LeNet5 network with several MLP layers 
at the end. Instead of training everything at the same time, we train each layer 
individually to see, if we can increase the accuracy (and maybe perform a training
of all layers together at the end).
...
def evaluate_lenet5(args, learning_rate=0, n_epochs=250,
    ...
    if unpickle_data is not '':
        ...
        datasets, size, size_out = load_SDSS_data(args, 500, reshape_2D = 0, wholespec=True, percentile = 100)
    ...
    ann_layout   = (2, nkerns[-1]*8*8, 11000, 11000, 11000, size_out)
    
    # variables for filenames of individual layer networks
    save_file_name       = ''
    save_best_file_name  = ''
    ...
    for m in xrange(len(ann_layout) - 2):

        if m == 0:
            ann_layout = (ann_layout[0], ann_layout[1], 5000, size_out)
            ...
            ann_layout = (size, 11000, size_out)
            classifier = MLP(
                rng=rng,
                input=x,
                activation=T.tanh,
            ...
            # convert the previous input to the new input, using the previously trained
            # layers of the network
            # on m == 1, we need to convert using the convolutional network
            if m == 1:
                data_new_x = convert_flux_to_prediction(None, size, train_set_x, valid_set_x, test_set_x, save_best_file_name, False)
            else:
                data_new_x = convert_flux_to_prediction(None, size, train_set_x, valid_set_x, test_set_x, save_best_file_name, True)
            ...
            try:
                ...
                for minibatch_index in xrange(n_train_batches):
                    ...
                    if (iter + 1) % validation_frequency == 0:
                        ...
                        print(
                            'layer %i, epoch %i, minibatch %i/%i, validation error %f %%' %
                            (
                                m,
                                epoch,
                                minibatch_index + 1,
        ...
        end_time = time.clock()
        print(('Optimization of layer %i complete. Best validation score of %f %% '
               'obtained at iteration %i, with test performance %f %%') %
              (m, best_validation_loss * 100., best_iter + 1, test_score * 100.))
        import sys
        print >> sys.stderr, ('The code for layer %i of file ' % (m) +
        ...
        save_file_name = './stacked_networks/stacked_layer'+str(m)+'.convmlp'
        save_best_file_name = './stacked_networks/stacked_best_layer'+str(m)+'.convmlp'
        save_file = open(save_file_name, 'wb')
        cPickle.dump(classifier.layer_params, save_file, -1)
        cPickle.dump(ann_layout, save_file, -1)
        save_file.close()
        # now pickle model with best parameters
        save_file = open(save_best_file_name, 'wb')
        cPickle.dump(best_params, save_file, -1)
        cPickle.dump(ann_layout, save_file, -1)
        save_file.close()
        
        stacked_layout.append(ann_layout)
        stacked_layer_params.append(layer_params)
        stacked_best_params.append(best_params)
    ...
    print >> sys.stderr, ('The code for file ' +
                          os.path.split(__file__)[1] +
                          ' ran for %.2fm' % ((end_total - start_total) / 60.))

    # cPickle classifier (better to pickle weights only!)
    # to use in predicting and plotting script
    # first cPickle final model
    save_file = open('stacked_classifier.convmlp', 'wb')
    cPickle.dump(stacked_layer_params, save_file, -1)
    cPickle.dump(stacked_layout, save_file, -1)
    save_file.close()
    # now pickle model with best parameters
    save_file = open('stacked_best_classifier.convmlp', 'wb')
    cPickle.dump(stacked_best_params, save_file, -1)
    cPickle.dump(stacked_layout, save_file, -1)
    save_file.close()    
