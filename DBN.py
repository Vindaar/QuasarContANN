...
class DBN(object):
    ...
    def __init__(self, numpy_rng, theano_rng=None, n_ins=784,
        ...
        print 'HiddenLayer output type', self.sigmoid_layers[-1].output.type
        print 'self.y type', self.y.type
        ...
        # basti: regression:
        self.finetune_cost = self.logLayer.prediction_error_sq(self.y)
        self.y_pred        = self.logLayer.y_pred
        self.y_pred_rbms   = self.sigmoid_layers[-1].output
...
def test_DBN(args, finetune_lr=0.01, pretraining_epochs=200,
    ...
    # save the parameters from after unsupervised training:
    unsupervised_params = dbn.params
    # cPickle classifier (better to pickle weights only!)
    # to use in predicting and plotting script
    # first cPickle unsupervised model
    save_file = open('classifier_unsupervised_params.dbn', 'wb')
    cPickle.dump(unsupervised_params, save_file, -1)
    ...
    patience = 16 * n_train_batches  # look as this many examples regardless
    ...
    print 'W[0]', dbn.params[0].eval()
    for param in dbn.params:
        print 'param', param.eval()

    #x = T.matrix('x')
    index = T.lscalar('index')
    predict_from_mlp = theano.function(
        inputs=[index],
        outputs=dbn.y_pred_rbms,
        givens={
            dbn.x: test_set_x[index:index+1]
        },
        on_unused_input='warn'
    )

    print 'predict unsupervised',
    print test_set_x[0,0].eval(), predict_from_mlp(0)
    ...
    # cPickle classifier (better to pickle weights only!)
    # to use in predicting and plotting script
    # first cPickle best model
    save_file = open('classifier_best_params.dbn', 'wb')
    cPickle.dump(best_params, save_file, -1)
    cPickle.dump(dbn_layout, save_file, -1)
    save_file.close()

    # now cPickle final model
    save_file = open('classifier_params.dbn', 'wb')
